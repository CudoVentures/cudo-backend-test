...
To get started, `yarn seed` in this folder to spin up a docker container running MySQL and automatically populate it with the data from `data/withdrawal-requests.json`. `yarn dev` will run `index.js`. Feel free to expand on the scaffold provided in `index.js`, or restructure the repository as you wish.
